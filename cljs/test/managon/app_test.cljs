...
(deftest initialize-db-sets-defaults
  (testing ":initialize-db populates every fact the Svelte scaffold held"
    ...
    (is (= "Ai etzhayyim Project Managon" @(rf/subscribe [:app/title])))
    (is (= "etzhayyim-project-managon" @(rf/subscribe [:app/name])))
    (is (= "etzhayyim-project-managon" @(rf/subscribe [:app/project])))
    ...
    (is (= "cljs/src/managon/app.cljs" @(rf/subscribe [:app/relative-path])))))
